...
| x86_64 `-smp 2` + NetSurf **triple-faults** (i386 `-smp 2` + NetSurf is fine; x86_64 UP is fine; x86_64 `-smp 2` passes faulttest/fdtest/socktest/polltest/musltest).  Confirmed PRE-EXISTING by re-testing the same scenario with the bounce-buffer work stashed | 🔲 **OPEN** — next SMP work item; no guest-side exception report reaches serial, so the fault happens where the handler itself cannot run (bad stack / AP context) |
