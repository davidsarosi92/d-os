...
| # | Finding | Status |
|---|---------|--------|
| 1.1 | Bad user pointer in any syscall = kernel #PF = halt | ✅ **FIXED** — two layers: a per-syscall gate (`task->in_user_syscall` + `vmm_user_access_ok`) and a real exception table (`.ex_table` + `uaccess_copy_in/out/str_in`, fixup in every arch fault handler).  `SYS_PRINT` and the whole `linux_abi` surface routed through it.  Plus a third layer (2026-08-01): **bounce buffers** — every bulk payload (read/write/send/recv/sendto/recvfrom/getdents/getdents64/poll/getrandom) is staged through a kernel chunk and the VFS/socket/console layers only ever see a `*_k` core with kernel memory, so a range going bad mid-transfer can no longer #PF in ring 0 where no fixup entry covers it.  Proof: `faulttest` (gate + fixup + bounce, incl. a copy that fails AFTER partial success).  **No residual** |
...
| `linux_abi`'s `sendmsg` gathers the client iovecs into a KERNEL buffer, then called the GATED `sys_write`/`sys_sendto` — which reject a kernel address while `in_user_syscall` is set.  musl's TCP-fallback DNS path had been broken since M46 | ✅ **FIXED** — calls the `sys_write_k`/`sys_sendto_k` cores |
| x86_64 `-smp 2` + NetSurf **triple-faults** as soon as a ring-3 task is load-balanced onto the AP.  Three per-CPU items were BSP-only: TR (`ltr`, and only one TSS descriptor existed), CR0/CR4 SSE enable, and EFER.SCE + the SYSCALL MSRs.  Confirmed PRE-EXISTING by reproducing with the bounce-buffer work stashed | ✅ **FIXED** — per-CPU TSS array + two GDT slots per CPU + `hal_arch_init_this_cpu()` (TR + SSE + SYSCALL MSRs) called from `ap_main`; verified `-smp 2`/`-smp 4` incl. NetSurf |
| x87/SSE register file is not saved across a context switch (pre-existing, documented on `enable_sse`).  Two concurrent SSE-using ring-3 tasks clobber each other on UP; on SMP a migrating task also resumes on a different core's XMM file | ✅ **FIXED** — per-task FXSAVE/FXRSTOR via a new HAL contract (`hal_fpu_init_state/save/restore`), a valid initial image at all four creation sites, and `fork` inheriting the parent's live state.  Proof: `fputest` reports `a=3000 b=0` mismatches with the fix removed, `a=0 b=0` with it |
