...
# d-os glue: the http/https fetcher.  NetSurf shipped here with only data:,
# resource: and file:, so the browser could not load a web page at all.  It
# attaches through fetcher_init's existing WITH_CURL hook — see the file header.
SRCS="$SRCS $U/netsurf/fetch_dos.c"

# --- compile ------------------------------------------------------------------
FAIL=0; OK=0
for src in $SRCS; do
    obj="$OUT/$(echo "$src" | sed 's,/,_,g').o"
    # WITH_CURL for content/fetch.c ALONE: it is what makes fetcher_init call
    # fetch_curl_register (which fetch_dos.c provides).  Globally it would also
    # switch utils/time.c to curl_getdate for HTTP dates — a symbol we do not
    # have, turning a working parser into a link error.
    case "$src" in
        */content/fetch.c) SRC_DEFS="$DEFS -DWITH_CURL" ;;
        *)                 SRC_DEFS="$DEFS" ;;
    esac
    if ! $CC -c $SRC_DEFS $INC "$src" -o "$obj" 2>"$OUT/err.txt"; then
