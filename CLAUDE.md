...
✅ **M1 – M15** shipped.  Done so far: kmalloc, driver registry
...
HID boot-keyboard class driver (root-port enumeration, Address Device,
Configure Endpoint, Event Ring polled from PIT IRQ)**.

🔲 **Next: §M16** — keyboard layout abstraction.  Both PS/2 and USB
HID drivers hardcode US QWERTY tables today; M16 introduces a
layered keycode → layout mapping so `setconf keyboard.layout hu`
swaps the layout without touching driver code.

🔲 Remaining: M16 keyboard layouts, M17 portability cut (hal_api.h),
M18 SMP, M19 memory at scale, M20 x64, M21 aarch64, M22 GUI.  See
PLAN.md "Active backlog" table for the full grid.
